...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("klksMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("klksMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("klksMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("klks-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running klksMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
